...
|---|---|---|
| 빌드 | | 경고 목록 |
| §1 테스트 10개 + Movement | | 실패 이름·`[Error]` 줄 |
| §2 실내 생성 | | door_1/door_out level 좌표 |
| §3 조명(키·F5·`golmok.tod`·에디터 대조) | | |
| §4 HUD·좌표(`stat fps/unit` 대조, ENU/lat/lon) | | gpu ms 값 |
| §5 포털 왕복(로그 순서·부호·재진입) | | |
| §5 음성 테스트(zone 삭제) | | |
| §5 경로 B(NamedStreamingLevel) | | |
| §6 충돌 표시 | | 와이어 머티리얼 출처 |
| §7 경로·CSV·golmok-perf 표 | | 표를 붙인다 |
| §8 스크린샷(`door.png` 이름) | | |
| §9 패키징 | | |
| §10 PIE 종료 | | |
| 고친 API(§11 번호) | | 커밋 해시 |
| 설계와 다른 동작 발견 | | 이 문서의 "설계와 다름" 외 추가분 |
