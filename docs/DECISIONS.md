...
- **남은 확인** (2026-09-24 PC 세션에서 실데이터로 확인한 것 포함):
  ...
  - 🔴 **국외 반출(법률 확인 필요, D-009)**: NGII 다운로드 신청서의 "사용자 준수사항"에 동의해야 받을 수 있고, 거기에 「공간정보관리법 제16조 및 제21조에 따라 국토교통부 장관의 허가 없이 측량성과를 국외 반출 시 2년 이하의 징역 또는 2천만원 이하의 벌금」이 적혀 있다. 법 제16조①: "누구든지 국토교통부장관의 허가 없이 기본측량성과 중 지도등 또는 측량용 사진을 국외로 반출하여서는 아니 된다. 다만, … 대통령령으로 정하는 경우에는 그러하지 아니하다." [국가법령정보센터, https://www.law.go.kr/법령/공간정보의구축및관리등에관한법률/제16조]. 정사영상·DEM 파생물(지형 텍스처, 스크린샷)을 **GitHub(국외 서버)에 올리거나 게임에 넣어 해외 배포**하는 것이 여기에 해당하는지는 판단하지 않았다 → 자문 전까지 파생 에셋은 PC에만 두고(`.gitignore`) 저장소·클라우드에 올리지 않는다.
