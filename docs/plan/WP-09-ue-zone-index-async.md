...
상태: 🟢 완료(PC 검증 통과, V-07 2026-09-26) (코드 2026-09-25, session_016UiCeoD2Sytfh4nSweonbs) · 담당: 클라우드 Claude 세션(**Fable 5.1 ultracode**, 검증 Opus 5.5, 모델 정책 DEVELOPMENT-PLAN §7.4) · 의존: WP-04, WP-05, WP-06 · 검증: G2(`runbooks/pc-verify-wp09.md`, V-03 뒤 PC 세션)
...
| 파괴 | `bSpawnedFromIndex && Unloaded && !bPinned && 3×3 밖 && 거리 > DespawnDistanceM(0 = 2×UnloadRadiusM, 사용 시점 계산) && !(실내 && 부모 Loaded/Loading)`가 **`DespawnGraceSeconds`(10 s) 연속** 성립할 때만. 배치 쌍둥이가 생긴 발견 zone은 `bRetirePending`으로 표시하고 `Unloaded`일 때 retire; Loaded/Loading이면 `Evaluate()`가 배치 쌍둥이가 Loaded이거나 플레이어가 언로드 반경 밖일 때 **pin과 무관하게** 언로드(취소)하고 그다음 발견 패스가 파괴(리뷰 수정: 원안은 `bManaged`에서 빼기만 해 영영 언로드되지 않았다; `FindZone`이 배치를 돌려주므로 콘솔·포털로는 풀 수 없다) |
...
| (126.9272636, 37.5620) | 16 | (55874, 25379) | `z_synthetic_002` 원점(001 동쪽 200 m; `make_index_fixture.py` 실측, 타원체 ENU → 7자리 반올림. 설계 초안의 126.9272665는 구면 근사값이었음) |
| (0, 0) | 16 | (32768, 32768) | |
...
| Idle→Pending / Leaving→Idle(`OnUnloadDelayElapsed`) / `EndPlay` | – | – | `bInteriorRequested=false; InteriorRequestSeconds=0; bActivated=false`(사이클마다 요청 시각을 새로 — 리뷰 수정) |
| Leaving, **`bActivated`**(`CompleteActivation`을 거친 Active에서 옴) | `BeginPlayerOverlap`(3 s 안 재진입) | – | WP-05 그대로 `Active`, `LastEvent="re-entered trigger; unload cancelled"` |
| Leaving, **`!bActivated`**(위 세 `State=Active; StartLeaving` 지름길 — 선로드 pin 반납 중) | `BeginPlayerOverlap`(3 s 안 재진입) | – | `Pending`, `LastEvent="re-entered trigger; interior still pending"`, `DebounceTimer` 재무장 `TimerRate(DebounceSeconds)` → `OnDebounceElapsed` → `IsInteriorReady` → `CompleteActivation`(요청·시각 유지, 재요청 없음). 리뷰 수정: 원안은 Active로 바로 가 StreamIn 없이·Loading 실내로 활성화됐다 |
| Idle | `golmok.portal enter`(`EnterInterior`→`Activate`) | Loading | 새 사이클(`bInteriorRequested=false; InteriorRequestSeconds=0` — 이전 방문의 시각으로 10 s 상한이 즉시 넘지 않게, 리뷰 수정) → `RequestInterior` + `IsInteriorReady ? CompleteActivation : (Pending 유지 + 폴링, bActivated=false)`; 메시지에 "loading; activates when ready" |
...
for (FGolmokZoneRecord& R : Zones) {
    ...
    if (bTwin) { R.bRetirePending = true; if (Z->State == Unloaded) ToRetire.Add(Z); continue; }  // pin 무시; Loaded/Loading 쌍둥이는 Evaluate()가 먼저 언로드(§4-5) → 다음 패스
...
for R in Zones:
  ...
  Loaded·Loading 공통(리뷰 수정): bRetireNow = R.bRetirePending && (bFar || (FindZone(id) != Zone && FindZone(id)->IsLoaded()))
      if (!R.bPinned && (…위 조건…)) || bRetireNow: ToUnload.Add(&R)     // 배치 쌍둥이가 대신함; pin 무시; 다음 DiscoverZones()가 파괴
  else: 기존 그대로
...
## 결과
세션: session_016UiCeoD2Sytfh4nSweonbs (Fable 5.1 ultracode; 심판·회의론자·수정 에이전트는 모델 정책대로 Opus 5.5) · 2026-09-25 · 상태 **🟢 완료** — PC 검증 V-07 통과(아래 단락). PR #15(병합) → V-07 PR.
...
**병합 전 보완 리뷰(오케스트레이터, Opus 읽기 전용, 2026-09-25)** — 블로킹 결함 없음(변경 C++·Python 전부 정독, MSVC C4458/C4456/C4459 스캔, 셀 공식·픽스처 바이트·런북 문자열 30여 개 대조, pytest 569·`make_index_fixture --check` OK). 컴파일 위험은 전부 런북 §11 표에 있음(#2 `RequestAsyncLoad` 오버로드, #11/#26 `SetTimerForNextTick` 반환형, #14 `SetManagerName`, #12 우선순위 상수는 `static_cast`라 odr-use 없음, #15 `GRenderThreadTimeCriticalPath`는 매크로 2일 때만). 병합 전에 반영한 것 3건: (a) `GetPlayerCell`이 `HasOrigin()`을 먼저 확인해 `AGolmokGeoOrigin` 없는 레벨(L_Dev)에서 `WarnNoOriginOnce`·"discovery needs …" 경고가 자동화 테스트마다 찍히지 않음(헤더 주석 "silently off"와 일치; 메시지는 Log 레벨) (b) `AcquireMeshAsset`이 `bAssetsPreloaded`와 무관하게 `ResolveObject()`를 먼저 시도 — 에디터에서 임포트만 하고 저장 안 한 메시가 `DoesPackageExist` 디스크 검사 때문에 와이어 박스로 빠지던 회귀 방지 (c) `zone_index.sync`가 index를 Content 안에 직접 빌드한 경우(`index_dir == dest`) `shutil.SameFileError` 대신 복사를 건너뜀(+테스트 1).
V-07 메모(코드 미변경): (1) `Golmok.Zone.IndexParse`는 의도된 Warning(`cell … lists z_synthetic_002@v2 …`)을 찍고 IndexDiscover는 002 에셋이 없어 chunk/collision 누락 Warning을 찍는다 — 테스트 결과는 "Success with warning"으로 기록(필요하면 `AddExpectedMessage`). (2) retire 경계: 발견 쌍둥이가 Loaded+pinned인 채 배치 쌍둥이가 사라지면 `bFar`로만 언로드되고, `Failed` 상태의 발견 zone은 파괴되지 않음(§5 관찰 시 참고). (3) 포털 타임아웃 Warning은 같은 폴링에서 `RequestInterior`가 불리면 Log로만 남을 수 있음(런북 §8은 Warning을 기대) — Log 줄로도 통과 처리. (4) 포털 leave의 `RequestUnload(Portal)`는 `bInteriorRequested`와 무관(WP-05 동작, 콘솔 pin 실내를 풀 수 있음; 낮은 우선순위). (5) `LonLatToCell`에 NaN이 들어오면 UB(플레이어 좌표는 NaN이 아님; 후속에 `std::isfinite` 가드). (6) IndexDiscover 실패 문구의 "level origin at 126.923 / 37.560"는 PlayerStart가 001 원점 5 m 남쪽이라는 뜻(코스메틱).

**PC 검증(V-07, 2026-09-26, PC 세션 Fable 5.1, 브랜치 `pc/v07-verify-wp09`, 결과 표 `runbooks/pc-verify-wp09.md` §12)** — **통과, C++ 수정 0건.** 클린 빌드 104 s에 컴파일·링크 오류 없음(§11 불확실 API 34행 전부 그대로; `RequestAsyncLoad` 고전 오버로드·`SetTimerForNextTick` `FTimerHandle` 반환 확인). 헤드리스 자동화 **16/16 Success**(Zone 5개는 의도된 Warning 포함 "Success with warning"), 재진입 Error 0, 엔진 `Couldn't find file for package` 0. §1 index 동기화는 커밋본과 바이트 동일(LF), §4 `L_ZoneTest09`에서 배치 액터 없이 BeginPlay +0.18 s에 3 zone 발견(001_interior→001→002 순서·`9 cells cached`·`near:` 순서 등 런북 문구 그대로), 001 비동기 로드 `34.8 ms wait + 1.0 ms build`(재로드 7.8~8.5 ms), 002 동기 폴백 와이어 박스, §5 1.5 km 동쪽에서 11.5 s 뒤 3건 동시 파괴(`18 cells cached, 14 missing`, `near: (none)`)·복귀 1.85 s 뒤 재발견, §8 포털 선로드(`priority 100`, 27 ms 뒤 로드) → `pending(loading …)` → Active, 실내 행 ` portal [placed]`, 디바운스 전 후퇴·3 s 안 재진입(Pending 재개, 바닥 있음)·`golmok.portal enter`·콘솔 회귀 전부 기대와 동일, §10 같은 틱 종료 `async load cancelled after 41.8 ms`·방 안/Pending 종료 뒤 잔류·dirty 없음. **§6 히치**(합성 zone, 60/100 m 반경, 같은 경로 재생, 워밍업 동일 90 s): async가 sync보다 나쁘지 않음 — 수치는 런북 §12 §6 표(001 재로드 순간 async는 p99 안, sync는 `loaded in 1.7 ms`지만 재로드 전후 수십 프레임이 11~17 ms). **§7 render ms**: HUD `render`는 `stat unit` Draw와 5/5 일치(±0.2 ms)이고 세 소스(0/1/2)가 같은 값(`begin`=`end` cycles)이라 **`GOLMOK_RENDER_TIME_SOURCE` 기본 1 유지**(코드 변경 없음). `0.00`은 우리 샘플링이 아니라 엔진 전역 `GRenderThreadTime` 자체가 0인 현상이다: `stat unit` Draw도 0.00이고, 엔진 CSV 프로파일러의 `RenderThreadTime` 열(같은 전역을 `UpdateCoreCsvStats_EndFrame`에서 읽음)도 PIE 세션에 따라 통째로 0.00(재생 e)이거나 4.7 ms(재생 C·D)다(한 세션 안에서 0 → 4.5 ms로 바뀐 경우도 있음, A 약 18 s). 엔진 `FSlateRHIRenderer`가 창(window) draw마다 `ThreadTime − Idle`로 덮어쓰는 구조라 에디터 PIE에서 창 상태에 따라 0이 되는 것으로 추정(엔진 쪽, 미확정) — `HoldLastPositive`는 0이 연속이면 잡을 값이 없어 효과 없음(held 1/2894). PC 발견: 런북 §4 맵 사본 코드가 `duplicate_asset` 뒤 같은 프로세스 `load_level`에서 GC Fatal → Save-As 코드로 교체(§11 #35); `test.ps1` 요약 `Succeeded` 계수(V-03과 동일); pytest는 `PYTHONUTF8=1` 필요(CI 동일). §9 패키징은 미실행. 스크린샷 `docs/images/pc-verify-wp09-*.jpg` 4장.

**남은 것**
- ~~PC 검증 V-07~~ → 통과(위 단락). 후속: §9 패키징(`DoesPackageExist` 쿠킹 동작), HUD `render` 0.00의 엔진 쪽 원인(PIE 첫 ~18 s, `stat unit`도 동일) — 필요하면 CSV 프로파일러가 읽는 시점(`UpdateCoreCsvStats_EndFrame`)과 같은 값을 쓰는 소스 후보.
