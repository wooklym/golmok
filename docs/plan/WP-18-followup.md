...
| GUI 조작 | `computer-use`로 Unreal 실행·창 확인 | EpicWebHelper의 Windows 보안 권한 창에서 중단. 보안 창 처리를 사용자에게 요청했으며, 권한 선택을 자동화하지 않음 |
...
- V-11: 실제 경로 폰 왕복·선택 복원 자동화 통과를 추가한다. GUI 보행·계단·포털 영상과 사진 통합은 아직 완료 표시하지 않는다.
