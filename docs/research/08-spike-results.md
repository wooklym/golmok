...
- 기기: 사용자 PC(Ryzen 5 7500F, 32GB, RTX 5060 8GB, 드라이버 591.86 → 617.14), UE 5.8.3, DX12 SM6.
...
| L_Dev 1920×1080, 드라이버 617.14 (2026-09-25) | 2999 | 160.7 | 114.5 | 6.01 | 8.73 | 2.05 | 6.91 | 5.77 |

- 드라이버 617.14: GPU 시간이 약간 줄었다(5.99 → 5.77 ms). 1% low는 CPU(렌더 스레드) 쪽 흔들림이라 회차마다 다르다. 업데이트 직후 첫 실행은 드라이버 셰이더 캐시를 다시 만들어 느리므로 비교에서 뺀다.
