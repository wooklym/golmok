...
WP-06: interior=True is the WP-05 fixture path only (door hole, SM_room, sublevel). Real zones go through
zone_import.run / interior_setup.run, which reuse the helpers of this module: find_or_spawn_geo_origin,
find_or_spawn_zone, open_or_create_level, _current_level_path, _spawn_interior_sublevel (with specs /
delete_tag / on_removed), sublevel_path, register_interior_sublevel(zone_id, version), boxes_glb and
expected_ue_bounds. run() keeps its signature (design D18).
"""
...
def _spawn_sublevel_actor(spec, location):
    ...
    if spec.get("tags"):  # interior_setup marks its actors (GolmokInteriorSetup) so re-runs replace only them
        actor.set_editor_property("tags", [unreal.Name(t) for t in spec["tags"]])
    return actor


def _spawn_interior_sublevel(
    int_manifest, zone_transform, return_level, specs=None, delete_tag=None, on_removed=None
):
    """Create (or reopen) the interior sublevel, place `specs` (default: sublevel_actor_specs) in *level*
    coordinates through the interior zone actor's transform, save it and reopen `return_level`. Returns the
    sublevel package path.

    Reopening an existing sublevel first removes the actors tagged `delete_tag` (interior_setup passes
    GolmokInteriorSetup, so hand-placed props survive) or, without a tag, those whose label is in `specs`;
    `on_removed(n)` is called with their count (only when the sublevel already existed)."""
    level_editor = unreal.get_editor_subsystem(unreal.LevelEditorSubsystem)
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(path):
        ...
        if delete_tag is not None:
            tag = unreal.Name(delete_tag)
            doomed = [a for a in actors.get_all_level_actors() if tag in list(a.get_editor_property("tags"))]
        else:
            labels = {s["label"] for s in specs}
            doomed = [a for a in actors.get_all_level_actors() if a.get_actor_label() in labels]
        for a in doomed:
            actors.destroy_actor(a)
        if on_removed is not None:
            on_removed(len(doomed))
